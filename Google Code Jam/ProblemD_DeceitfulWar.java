package Google_CodeJam_Qualification_2014;

import java.util.Arrays;
import java.util.Scanner;
import java.util.TreeSet;

public class ProblemD_DeceitfulWar {
	...
	private static int playDecWar(double[] a, double[] b) {
		Arrays.sort(a);
		Arrays.sort(b);
		int ret = 0;
	...
	private static int playDecWar(double[] a, double[] b, int fakeout) {
		int ret = 0;
		int n = a.length;
	...
	private static int playWar(double[] a, double[] b) {
		Arrays.sort(a);
		TreeSet<Double> left = new TreeSet<Double>();
		int ret = 0;
